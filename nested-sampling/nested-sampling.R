...
fileConn = file("nested-sampling/ns-output.csv", open="w")
...
while(TRUE)
{
    ...
    # Save its details
    fileConn = file("nested-sampling/ns-output.csv", open="a")
    line = ""
    ...
    if(iteration %% num_particles == 0)
    {
        ...
        png(filename="nested-sampling/ns-progress-plot.png", width=1000, height=1000)

        keep = as.matrix(read.csv("nested-sampling/ns-output.csv"))
...
m <- paste("Marginal likelihood: ln(Z) = ", logZ, " +- ", err, ".", sep="")
...
write.table(posterior_samples, file="nested-sampling/ns-posterior-samples.csv",
            sep=",", row.names=FALSE, col.names=TRUE, quote=FALSE)
cat("Posterior samples saved in ns-posterior-samples.csv.\n")

ns <- list(posterior_samples = data.frame(posterior_samples), Z = m)
